...
import { getGmailCredentials, saveGmailCredentials, getAllGmailCredentials } from "./redis";
...
export const authenticateGmail = async (email: string): Promise<GmailAuthResult | null> => {
  try {
    ...
    console.log(`Got credentials for ${email}, expires at: ${credentials.expiresAt ? new Date(credentials.expiresAt * 1000).toISOString() : 'unknown'}`);
    ...
    console.error(`Unexpected error during Gmail authentication for ${email}:`, error);
    if (error instanceof Error) {
      console.error('Error details:', {
        message: error.message,
        name: error.name,
...
export const getLabels = async (gmail: gmail_v1.Gmail): Promise<Map<string, string>> => {
...
export const processUserEmails = async (email: string): Promise<{
  email: string;
  success: boolean;
  ...
  try {
    const auth = await authenticateGmail(email);
    if (!auth) {
      return {
        email,
        success: false,
    ...
    const emails = await Promise.all(
      threadResponse.data.threads.map(async (thread) => {
        ...
        const message = threadDetails.data.messages?.[threadDetails.data.messages.length - 1];
        if (!message || !message.payload?.headers) return null;

        const headers = message.payload.headers;
        const body = message.payload.parts?.find((part) => part.mimeType === "text/plain");
        const base64Data = (body?.body?.data || "")
          .replace(/-/g, "+")
          .replace(/_/g, "/")
          .padEnd((body?.body?.data || "").length + ((4 - ((body?.body?.data || "").length % 4)) % 4), "=");
        ...
        return {
          ...
          bodyText: bodyText.length > 10000 ? message.snippet + "..." : bodyText,
        };
      })
    );
    ...
    // Ensure labels exist
    const labelIds = await ensureLabelsExist(gmail, ["To Read", "To Reply", "To Archive"]);
    ...
    const processedResults = results.filter((r): r is NonNullable<typeof r> => r !== null);
    ...
    console.error(`Error processing emails for ${email}:`, error);
    return {
      email,
      success: false,
